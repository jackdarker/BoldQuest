...
func on_setupScene():
	if(self.parent_scene.args[0] is LootContainer):
		loot=self.parent_scene.args[0].getLoot()
		self.parent_scene.args[0].lootContainer()
		
	if(true && loot.size()>0): # TODO if no trap we skip scene
...
func get_buttons(menuid:String,buttons:Array):
	if(menuid==""):
		...
		if(loot.size()>0):
			for _item in loot:
				buttons.push_back(Button_Config.new("take "+_item.getName(),"",pickup.bind(_item)))
		else:
